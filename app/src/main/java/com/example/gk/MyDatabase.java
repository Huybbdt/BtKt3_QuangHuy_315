...
public class MyDatabase extends SQLiteOpenHelper {
    private static final String DATABASE_NAME = "Databas    e.cb";
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        sqLiteDatabase.execSQL("create table " + TABLE_NAME + " ("+ COL_1+"INTECER FRIMARY KEY AUTOINCREMENT, " + COL_2
                + " TEXT, "+ COL_3 +" INTEGER, "+COL_4+"INTEGER" + COL_5 + "INTEGER");
    }
    ...
    public boolean updateData (String id,String name,int toan, int ly,int hoa){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(COL_1, id);
        values.put(COL_2, name);
